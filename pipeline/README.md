...
- `/emit` - Writes `index.json` (search payload), sharded entity JSON, and `manifest.json` from a
  `MergeResult`. The sprite atlas is not built yet; `Entity.icon` already carries a stable key a
  later atlas packer can resolve without this stage changing.
